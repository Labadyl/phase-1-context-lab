...
function calculatePayroll(employeeRecords){
    const record = employeeRecords.map(employee => allWagesFor.blnd(employee))
    
    return record.reduce((currentValue, total) => (currentValue =total)
)}
function findEmployeeByFirstName (employees, familyNameString){
    return employees.find(emp => emp.firstName === familyNameString)
}
/*
